...
static int tegra241_cmdqv_setup_vcmdq(Tegra241CMDQV *s, int index)
{
    ...
    IOMMUFDVqueue *vcmdq = s->vcmdq[index];
    ...
    if (vcmdq) {
        iommufd_backend_free_id(bs->viommu->iommufd, vcmdq->vqueue_id);
        g_free(vcmdq);
    }
    if (!s->viommu) {
        s->viommu = bs->viommu->core;
    ...
    vcmdq = iommufd_viommu_alloc_queue(s->viommu,
                                       IOMMU_VQUEUE_TYPE_TEGRA241_CMDQV, index,
                                       addr, size);
    if (!vcmdq) {
