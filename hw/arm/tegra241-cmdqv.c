...
static int tegra241_cmdqv_setup_vcmdq(Tegra241CMDQV *s, int index)
{
    ...
    ret = smmu_iommu_set_viommu_data(bs, IOMMU_VIOMMU_DATA_NVGRACE_VCMDQ,
                                     sizeof(data), (void *)&data);
    if (ret) {
        error_report("failed to setup VCMDQ%d", index);
        return ret;
    }
                                       ...
                                       int index, uint64_t value, unsigned size)
{
    ...
    case A_VCMDQ0_CONFIG:
        ptr = (uint32_t *)(s->vcmdq_page0 + 0x80 * index + offset - 0x10000);
        if (!(*ptr & R_VCMDQ0_CONFIG_CMDQ_EN_MASK) &&
            (value & R_VCMDQ0_CONFIG_CMDQ_EN_MASK)) {
                                 ...
                                 uint64_t value, unsigned size)
{
    ...
    case A_VI_VCMDQ0_CONS_INDX ... A_VI_VCMDQ127_GERRORN:
        /*
         * Align offset down to 0x30000 while extracting the index:
         *   VI_VCMDQ0_CONS_INDX  (0x30000) => 0x30000, 0
         *   VI_VCMDQ1_CONS_INDX  (0x30080) => 0x30000, 1
         *   VI_VCMDQ2_CONS_INDX  (0x30100) => 0x30000, 2
         *   ...
         *   VI_VCMDQ127_CONS_INDX (0x33f80) => 0x30000, 127
         */
        index = (offset - 0x30000) / 0x80;
        tegra241_cmdqv_write_vcmdq(s, offset - 0x80 * index, index, value, size);
        break;

    case A_VI_VCMDQ0_BASE_L ... A_VI_VCMDQ127_CONS_INDX_BASE_DRAM_H:
        /*
         * Align offset down to 0x40000 while extracting the index:
         *   VI_VCMDQ0_BASE_L  (0x40000) => 0x20000, 0
         *   VI_VCMDQ1_BASE_L  (0x40080) => 0x40000, 1
         *   VI_VCMDQ2_BASE_L  (0x40100) => 0x40000, 2
         *   ...
         *   VI_VCMDQ127_BASE_L (0x43f80) => 0x40000, 127
         */
        index = (offset - 0x40000) / 0x80;
        tegra241_cmdqv_write_vcmdq(s, offset - 0x80 * index, index, value, size);
        break;
