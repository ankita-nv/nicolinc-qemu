...
{
    struct egm_retired_pages_info *page1 = (struct egm_retired_pages_info *) a;
    struct egm_retired_pages_info *page2 = (struct egm_retired_pages_info *) b;
...
static int add_memory_regions(struct egm_retired_pages_list *info, void *fdt,
                              ...
                              hwaddr mem_len, int numa_node_id)
{
    ...
    qsort(info->retired_pages, info->count,
          sizeof(struct egm_retired_pages_info), &offset_compare);
    mem_last_curr = mem_len;
    for (index = info->count - 1; index >= 0; index--) {
        mem_base_curr = info->retired_pages[index].offset +
                        info->retired_pages[index].size;
        ...
        mem_last_curr = info->retired_pages[index].offset;
    }
...
{
    size_t argsz = sizeof(struct egm_retired_pages_list);
    int fd;
...
retry:
    ...
    if (ioctl(fd, EGM_RETIRED_PAGES_LIST, *info)) {
        g_free(*info);
        *info = NULL;
        return;
    }
    ...
    if (ms->numa_state != NULL && ms->numa_state->num_nodes > 0) {
        ...
        for (i = 0; i < ms->numa_state->num_nodes; i++) {
            ...
            if (ms->numa_state->nodes[i].node_memdev) {
                struct egm_retired_pages_list *info = NULL;
